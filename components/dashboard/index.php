...
<section>
  <section class="nav-month">
    ...
    <h3 class="center-block"><?php echo monthToString($data['month']['m']);?></h3>
...
var doubleChart = new Chart(doubleChartElement, {
    ...
    data: {
        labels: ["Usage", "Income"],
        datasets: [
          <?php foreach (sanitizeUsageTwo($data['categories'], 'income', 'usage') as $key => $value): ?>
          {
            label: ["<? echo html_entity_decode($value['name'] . ", " . $value['name']);?>"],
            data: [<? echo $value['usage'] . ", " . $value['income'];?>],
            backgroundColor: "<?php echo $value['colors'][0]?>",
            borderColor: "<?php echo $value['colors'][1];?>",
            borderWidth: 1
        } ,
