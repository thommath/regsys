  ...
  if(!isset($_POST['sum']) || !isset($_POST['category']) || !isset($_POST['date']) || !isset($_POST['id'])){
    ...
    if(!$accepted){
        array_push($errorList, "Ohh no! It seems like there is a bug in here? It's hiding, can you help me catch it by emailing us? Thank you :)");
            array_push($errorList, "UPDATE `Bill` SET `description`='" . $_POST['description'] . "', `category`='" . $_POST['category'] . "', `sum`='" . $_POST['sum'] . "', `date`='" . $_POST['date'] . "' WHERE `user`=" . $_SESSION['user'] . " AND `id`=" . $_POST['id']);
        $_SESSION['from'] = 'editBill';
        $_SESSION['success'] = false;
        $_SESSION['error'] = $errorList;
        $_SESSION['post'] = $_POST;
        header("Location: http://" . $_SERVER['SERVER_NAME'] . "?p=editBill", true);
        die();
    }else{
        $_SESSION['from'] = 'editBill';
